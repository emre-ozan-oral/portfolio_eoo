...
import { Difficulty, Puzzle, generatePuzzle } from "./zipEngine";
...
const SPEEDS = [
  { key: "slow", label: "Slow", ms: 700 },
  { key: "normal", label: "Normal", ms: 300 },
  { key: "fast", label: "Fast", ms: 90 },
] as const;
...
export default function SolverView() {
  ...
  const path = useMemo(() => step?.path ?? [], [step]);
  ...
  return (
    <div className="flex flex-col items-center gap-8 w-full">
      ...
      <p className="text-[var(--dim)] text-[10px] tracking-[0.1em] text-center max-w-md" style={{ fontFamily: "var(--font-mono)" }}>
        The trailing color shows the path so far; arrows show which way it continues.
      </p>
    </div>
  );
}
