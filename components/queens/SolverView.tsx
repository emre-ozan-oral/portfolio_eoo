...
const SPEEDS = [
  ...
  { key: "normal", label: "Normal", ms: 550 },
  { key: "fast", label: "Fast", ms: 180 },
] as const;
...
const KIND_LABEL: Record<SolverStep["kind"], string> = {
  "row-forced": "Row rule",
  "col-forced": "Column rule",
  "region-forced": "Region rule",
  "line-elimination": "Line elimination",
  fallback: "Fallback",
  win: "Solved",
};
...
export default function SolverView() {
  ...
  const marksAtStep = useMemo(() => step?.marks ?? (puzzle ? emptyMarks(puzzle.n) : []), [step, puzzle]);

  const highlights = useMemo(() => {
    if (!step) return undefined;
    const map: Record<string, CellHighlight> = {};
    for (const [r, c] of step.sourceCells) map[`${r},${c}`] = "source";
    for (const [r, c] of step.targetCells) map[`${r},${c}`] = "target";
    return map;
  }, [step]);
  ...
  return (
    <div className="flex flex-col items-center gap-8 w-full">
        ...
        <div className="w-full lg:w-[280px] flex flex-col gap-2">
          ...
          <div ref={logRef} className="h-[220px] overflow-y-auto border border-[var(--border)] bg-[var(--surface)]/60 p-3 flex flex-col gap-2">
            {steps.slice(0, stepIndex + 1).map((s, i) => (
              ...
              >
                <span className="text-[var(--accent)]">{KIND_LABEL[s.kind]}</span> — {s.message}
              </p>
            ))}
          </div>
        </div>
      ...
      <p className="text-[var(--dim)] text-[10px] tracking-[0.1em] text-center max-w-md" style={{ fontFamily: "var(--font-mono)" }}>
        Gold outline = the region/line driving the deduction. Green outline = the cell(s) it resolves.
      </p>
    </div>
  );
}
